...
def assess_strategy_dataframe(trades, start_date, end_date, starting_value = 1000000, fixed_cost = 9.95, floating_cost = 0.005):
    ...
    #get stock data
    stocks_vals = get_data(start_date, end_date, symbol)
    
    #shares = stocks_vals.index
    shares = pd.DataFrame(index=stocks_vals.index)
    cash_val = pd.DataFrame(index=stocks_vals.index)
    cash_val.loc[start_date: end_date, "Cash"] = starting_value
    shares.loc[start_date: end_date, symbol] = 0
    ...
    for day in trades.index:
        ...
        print(total_transaction_value)
        
        if trade_shares > 0:
            shares.loc[day : end_date, symbol] += trade_shares
            cash_val.loc[day : end_date, "Cash"] -=  total_transaction_value*(1 + floating_cost) + fixed_cost
        elif trade_shares < 0:
            shares.loc[day : end_date, symbol] -= trade_shares
            cash_val.loc[day : end_date, "Cash"] +=  total_transaction_value*(1 - floating_cost) - fixed_cost
    print(cash_val)
...
def calc_portfolio(portfolio_val, starting_value = 1000000, risk_free_rate = 0.0, sample_freq = 252):
    end_value = portfolio_val["Portfolio"].iloc[-1]
    average_daily_return = ((portfolio_val["Portfolio"]/ portfolio_val["Portfolio"].shift()) - 1).mean()
